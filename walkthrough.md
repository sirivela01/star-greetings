...
- **Expectiminimax Bot AI Engine**: Replaced the basic rule-based Bot decision heuristics with a **1-step lookahead Expectiminimax search tree** solver. The Bot now clones the game state internally and simulates actions, branching across all possible next dice rolls (1-6) as weighted chance nodes. The leaf states are scored using a comprehensive evaluation function (evaluating positional ring progress, yard penalties, safety on safe tiles, vulnerability to capture, and threatening positions), making the Bot play with high-level game theory strategy.
- **Demon-Themed Game Pieces**: Replaced the 3D sphere rocks with gorgeous, diamond-encrusted spinning top demons of different colors (Red/Bronze, Green, Yellow/Gold, and Blue/Silver) directly extracted from the uploaded images.
- **4-Player Gameplay Support**: Fully generalized the Barakatta game engine (`game-logic.js`, `ui-rendering.js`, `index.html`, and `styles.css`) to support all 4 players (Red, Green, Yellow, Blue) on the board simultaneously.
  - In Solo vs AI: Player 1 (Red) vs 3 Bots (Green Bot, Yellow Bot, Blue Bot).
  - In Offline Pass & Play: 4 Human Players taking turns clockwise around the board.
- **Side Panel Control Panel Update**: Upgraded the sidebar panel to a grid showing the yard rocks and home progress of all 4 players concurrently.
- **Generalization of Game Logic**: Custom turn orders, dynamic safety/threat AI heuristics, and win check routines now scale smoothly to any active players.
